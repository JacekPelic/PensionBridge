...
import { calculateAllEstimates, totalPillar1Eur, COUNTRY_META } from '@/modules/pension';
...
export function StepGap({ state, onChange }: Props) {
  ...
  const grossPillar1Total = totalPillar1Eur(estimates);
  const residenceCountry = (state.residenceCountry ?? 'LU') as ResidenceCountry;
  const { netAnnual, effectiveRate } = calculateTax(grossPillar1Total * 12, residenceCountry);
  const pillar1Total = Math.round(netAnnual / 12);
  ...
  return (
    <div>
      ...
      <p className="text-sm mb-8 max-w-[540px]" style={{ color: 'var(--text-muted)' }}>
        Your State pension alone covers part of your retirement income. After {Math.round(effectiveRate * 100)}% estimated tax,
        the gap represents what your Workplace pension and Personal savings need to fill.
      </p>
        ...
        <div className="rounded-xl p-5 mb-5" style={{ background: 'var(--navy-2)', border: '1px solid var(--border)' }}>
            ...
            >
              {pct >= 15 && (
                <span className="text-[10px] font-bold" style={{ color: 'var(--navy)' }}>
                  State: {'\u20AC'}{pillar1Total.toLocaleString()}
                </span>
              )}
            </div>
            {gap > 0 && (
          ...
          <div className="flex gap-3 mt-3">
            {estimates.map((est) => {
              ...
              const { netAnnual: cNet } = calculateTax(est.monthlyPensionEur * 12, residenceCountry);
              const netMonthly = Math.round(cNet / 12);
              const countryPct = goal > 0 ? Math.round((netMonthly / goal) * 100) : 0;
              return (
                <div key={est.country} className="flex items-center gap-1.5 text-[11px]" style={{ color: 'var(--text-muted)' }}>
                  <span>{meta.flag}</span>
                  <span>{'\u20AC'}{netMonthly.toLocaleString()} net</span>
        ...
        {gap > 0 ? (
          <div className="rounded-xl p-5" style={{ background: 'var(--navy-2)', border: '1px solid var(--border)' }}>
            ...
            <div className="text-[12px] mt-3" style={{ color: 'var(--text-muted)' }}>
              This gap can be covered through Workplace pensions and Personal
          ...
          <div className="rounded-xl p-5 text-center" style={{ background: 'var(--green-dim, rgba(34,197,94,0.08))', border: '1px solid var(--green)' }}>
            <div className="text-[15px] font-semibold" style={{ color: 'var(--green)' }}>
              Your State pension income covers your goal!
            </div>
            <div className="text-[12px] mt-1" style={{ color: 'var(--text-muted)' }}>
              +{'\u20AC'}{Math.abs(gap).toLocaleString()}/mo surplus. Workplace &amp; Personal savings will add even more.
            </div>
          </div>
        )}
      </div>
